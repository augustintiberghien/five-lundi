#!/usr/bin/env python3
"""s10 : re-échange GK — Rémi → Blanche(50,8), Michael → Bleue(50,92)"""
...
def swap(content, s10_s, s9_s, name, ox, oy, ot, nx, ny, nt):
    ...
    if pos < 0 or pos > s9_s: print(f"❌ {name}"); return content
    ...
    if 0 < cp < s9_s2: content = content[:cp] + new_cls + content[cp+len(old_cls):]
    return content

c = swap(c, s10_start, s9_start, 'Michael', 50,  8, 'true',  50, 92, 'false')
s10_start = c.find("id:'s10'"); s9_start = c.find("id:'s9'")
c = swap(c, s10_start, s9_start, 'Rémi',    50, 92, 'false', 50,  8, 'true')

with open('index.html', 'w') as f:
    f.write(c)
print("✅ Rémi(Blanche GK) · Michael(Bleue GK) — ΔSM=2 · ΔNote=0.5")
